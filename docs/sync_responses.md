...
EXPERIMENTAL: The features outlined in this document are considered experimental and are therefore subject to change outside of major version releases.

In a regular Benthos stream pipeline messages flow in one direction and acknowledgements flow in the other:
...
For most Benthos input and output targets this is the only workflow available. However, Benthos has support for a number of protocols where this limitation is not the case.
...
It's possible to route the result of any Benthos processing pipeline directly back to an input with a [`sync_response`](./outputs/README.md#sync_response) output:
...
It's also possible to combine a `sync_response` output with other outputs using a [`broker`](./outputs/README.md#broker):
...
NOTE: It's safe to use these mechanisms even when combining multiple inputs with a broker, a response payload will always be routed back to the original source of the message.

Routing Output Responses Back
-----------------------------

Some outputs, such as [`http_client`](./outputs/README.md#http_client), have the potential to propagate payloads received from their destination after sending a message back to the input:
