...
- No checkpoint *schema version bump* needed — `user_name` never touches the SCN-based checkpoint/positioning cache. **Correction (found during final review, not anticipated when this plan was written):** the `cache_resource` transaction-buffer mode *does* persist in-flight `DMLEvent`s to a DB-backed cache via `serializedDMLEvent` in `internal/impl/oracledb/logminer/cache_resource.go`, and that struct needed `UserName` added to its marshal/unmarshal alongside the other fields (Task 4 discovered and fixed this by running the real integration suite against a live Oracle container — the in-memory-cache-only unit tests from Tasks 1-3 never exercised this path). This is an additive, unversioned JSON field, safe in both directions (old rows read as `""` → absent metadata, matching documented semantics).
