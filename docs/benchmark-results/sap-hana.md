...
Write-path benchmarks are not included here: the dedicated `sap_hana` output measured in earlier drafts was removed from this PR (writes go through `sql_insert` with `driver: hana` instead), and the corresponding harness will land with the write-path PR so the numbers stay reproducible.
