...
func TestTaggingErrorsSinglePart(t *testing.T) {
	...
	// No error
	assert.Nil(t, tran.resFromError(nil))
	...
	assert.Nil(t, tran.resFromError(batchErr))
...
func TestTaggingErrorsNestedOverlap(t *testing.T) {
	...
	// No error
	assert.Nil(t, tranOne.resFromError(nil))
	assert.Nil(t, tranTwo.resFromError(nil))
...
func TestTaggingErrorsNestedSerial(t *testing.T) {
	...
	assert.Equal(t, errTest2, tranOne.resFromError(batchErr))
	assert.Nil(t, tranTwo.resFromError(batchErr))
}
