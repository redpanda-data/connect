...
//
// Whether the signal's own position is acknowledged before or after its
// action completes is a connector-specific choice: acking before (once
// delivery is confirmed) is simpler and never redelivers, at the cost of
// losing the action if a crash lands between the ack and the action
// finishing; acking after avoids that window but means a source that
// redelivers unacked events will keep resending the signal until Listen
// recognizes and drops the redelivery itself.
//
