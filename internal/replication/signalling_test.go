...
func TestControlSignal_FilterTables(t *testing.T) {
	tests := []struct {
		name            string
		dataCollections []string
		schema          string
		...
		{
			name:            "empty data-collections returns all tables unchanged",
			dataCollections: nil,
			schema:          "dbo",
			tables:          []string{"events", "products"},
			want:            []string{"events", "products"},
		},
		{
			name:            "filters to matching table",
			dataCollections: []string{"dbo.events"},
			schema:          "dbo",
			tables:          []string{"events", "products"},
			want:            []string{"events"},
		...
		{
			name:            "no match in DBTables returns empty slice",
			dataCollections: []string{"dbo.unknown"},
			schema:          "dbo",
			tables:          []string{"events", "products"},
			want:            []string{},
		},
		{
			name:            "cross-schema entry matches nothing in DBTables",
			dataCollections: []string{"other.events"},
			schema:          "dbo",
			tables:          []string{"events", "products"},
			want:            []string{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := &ControlSignal{DataCollections: tt.dataCollections}
			got := s.FilterTables(tt.schema, tt.tables)
			assert.Equal(t, tt.want, got)
		})
	}
}
