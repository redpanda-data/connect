...
-----END EC PRIVATE KEY-----`
	...
	for _, tc := range testCases {
		tc := tc
		t.Run(tc.method+"_headers", func(t *testing.T) {
			mapping := fmt.Sprintf("root = this.%s(signing_secret: %q, headers: {\"kid\": \"1234\", \"alg\": \"none\", \"typ\": \"BAD\", \"x\": 123})", tc.method, tc.secret)
			...
			if v, ok := tok.Header["kid"]; ok {
				require.Equal(t, "1234", v)
			} else {
				require.Fail(t, "kid header missing")
			}

			if v, ok := tok.Header["x"]; ok {
				require.Equal(t, float64(123), v)
			} else {
				require.Fail(t, "x header missing")
			}
	...
	mapping := fmt.Sprintf("root = this.%s(signing_secret: %q, headers: \"not-an-object\")", "sign_jwt_hs256", dummySecretHMAC)
	exe, err := bloblang.Parse(mapping)
	require.NoError(t, err)
