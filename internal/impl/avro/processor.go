...
specified encoding.`).
		...
		Field(service.NewStringField("schema_path").
			Description("The path of a schema document to apply. Use either this or the `schema` field. URLs must begin with `file://` or `http://`. Note that `file://` URLs must use absolute paths (for example, `file:///absolute/path/to/spec.avsc`); relative paths are not supported.").
			ShortDescription("Path of a schema document to apply. Use either this or the schema field.").
			Default("").
