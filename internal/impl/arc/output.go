...
const (
	aoFieldToken          = "token"
	aoFieldDatabase       = "database"
	aoFieldMeasurement    = "measurement"
	aoFieldFormat         = "format"
	aoFieldTimestampField = "timestamp_field"
	aoFieldTimestampUnit  = "timestamp_unit"
	aoFieldTagsMapping    = "tags_mapping"
	aoFieldCompression    = "compression"
	aoFieldBatching       = "batching"
)
...
` + service.OutputPerformanceDocs(true, true))
	...
	return spec.Fields(
		...
		service.NewStringField(aoFieldDatabase).
			Default("default").
			Description("The target database name."),
		service.NewInterpolatedStringField(aoFieldMeasurement).
...
func init() {
	service.MustRegisterBatchOutput("arc", outputSpec(),
		func(conf *service.ParsedConfig, mgr *service.Resources) (out service.BatchOutput, batchPol service.BatchPolicy, mif int, err error) {
			if batchPol, err = conf.FieldBatchPolicy(aoFieldBatching); err != nil {
				return
			}
...
func newArcOutput(conf *service.ParsedConfig, mgr *service.Resources) (*arcOutput, error) {
	...
	if conf.Contains(aoFieldToken) {
		token, err := conf.FieldString(aoFieldToken)
		if err != nil {
			return nil, err
		}
	...
	if o.database, err = conf.FieldString(aoFieldDatabase); err != nil {
		return nil, err
	}
	...
	if o.measurement, err = conf.FieldInterpolatedString(aoFieldMeasurement); err != nil {
		return nil, err
	}

	if o.format, err = conf.FieldString(aoFieldFormat); err != nil {
		return nil, err
	}

	if o.timestampField, err = conf.FieldString(aoFieldTimestampField); err != nil {
		return nil, err
	}

	if o.timestampUnit, err = conf.FieldString(aoFieldTimestampUnit); err != nil {
		return nil, err
	}

	if conf.Contains(aoFieldTagsMapping) {
		if o.tagsMapping, err = conf.FieldBloblang(aoFieldTagsMapping); err != nil {
			return nil, err
		}
	}

	if o.compression, err = conf.FieldString(aoFieldCompression); err != nil {
		return nil, err
	}
