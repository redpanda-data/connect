...
root.data = this`)
	...
	bloblang.MustRegisterFunctionV2(
		"buffer_acquire",
		bloblang.NewPluginSpec().
			Category("General").
			Description("Acquires a byte buffer from a named pool. Buffers are reused across invocations to reduce allocations. If no buffer is available in the pool, a new one of the specified size is created. For a given pool, the size must never change. Always release the buffer with `buffer_release` when done to return it to the pool.").
			Param(bloblang.NewStringParam("pool").Description("The name of the buffer pool to acquire from.")).
			Param(bloblang.NewInt64Param("size").Description("The size of the buffer to allocate if none are available in the pool.")).
			Impure().
			Static(),
		func(args *bloblang.ParsedParams) (bloblang.Function, error) {
			poolName, err := args.GetString("pool")
			if err != nil {
				return nil, err
			}
			...
			v, ok := bufferPools.Load(poolName)
			if !ok {
				v, _ = bufferPools.LoadOrStore(poolName, &sync.Pool{New: func() any {
	...
	bloblang.MustRegisterFunctionV2(
		...
		bloblang.NewPluginSpec().
			Category("General").
			Description("Returns a byte buffer to a named pool for reuse. The buffer should have been previously acquired with buffer_acquire from the same pool. Returns null.").
			Param(bloblang.NewStringParam("pool").Description("The name of the buffer pool to return the buffer to.")).
			Param(bloblang.NewAnyParam("buffer").Description("The byte buffer to return to the pool.")).
			Impure(),
		func(args *bloblang.ParsedParams) (bloblang.Function, error) {
			poolName, err := args.GetString("pool")
			if err != nil {
			...
			raw, err := args.Get("buffer")
			if err != nil {
				return nil, err
			}
			_, ok := raw.([]byte)
			if !ok {
				return nil, fmt.Errorf("buffer_release expected bytes, got %T", raw)
			}
			return func() (any, error) {
				rawPool, ok := bufferPools.Load(poolName)
				if !ok {
					return nil, fmt.Errorf("buffer pool %q not found", poolName)
				}
