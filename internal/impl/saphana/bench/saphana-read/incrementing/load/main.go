...
func main() {
	count := flag.Int("count", 100000, "number of rows to insert")
	batch := flag.Int("batch", 5000, "rows per transaction")
	workers := flag.Int("workers", 10, "number of parallel insert workers")
	setupOnly := flag.Bool("setup-only", false, "drop and recreate empty table then exit")
	maxID := flag.Bool("max-id", false, "print MAX(ID) from table then exit")
	flag.Parse()
	...
	table := fmt.Sprintf(`"%s"."BENCH_ORDERS_INC"`, schema)
	...
	if *setupOnly {
		...
		fmt.Printf("Table %s created (empty)\n", table)
		return
	}

	if *maxID {
		var id sql.NullInt64
		row := db.QueryRowContext(ctx, `SELECT MAX(ID) FROM `+table)
		if err := row.Scan(&id); err != nil {
			fmt.Fprintf(os.Stderr, "failed to query max ID: %v\n", err)
			os.Exit(1)
		}
		if id.Valid {
			fmt.Printf("%d\n", id.Int64)
	...
	effectiveWorkers := min(*workers, *count)
	...
	for w := range effectiveWorkers {
		...
		go func(workerIdx int) {
			...
			for batchStart := startRow; batchStart < endRow; batchStart += *batch {
				batchEnd := min(batchStart+*batch, endRow)
