...
//
// Responsibilities:
// - Resolve destination topic names from source names using an optional interpolated template.
// - Create destination topics mirroring source partition count and a selected replication factor.
// - Copy a supported subset of topic configurations (reduced set when serverless is enabled).
// - Optionally synchronise ACLs from source to destination topics.
// - Cache known topics to avoid redundant lookups and creations.
// - Record metrics and emit structured logs for observability.
