...
func TestParquetParseBloblangV2AsStrings(t *testing.T) {
	...
	exec, err := bloblangv2.Parse(`output = input.parse_parquet(byte_array_as_string: true)`)
	require.NoError(t, err)
...
func TestParquetParseBloblangV2InvalidInput(t *testing.T) {
	exec, err := bloblangv2.Parse(`output = input.parse_parquet()`)
	require.NoError(t, err)
