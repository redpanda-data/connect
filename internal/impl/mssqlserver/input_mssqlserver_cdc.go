...
- seqval (The position of the change in the transaction log, from the change table column ` + "`__$seqval`" + `, encoded as a hexadecimal string with a ` + "`0x`" + ` prefix. Not present on snapshot (` + "`read`" + `) messages.)
- command_id (The order of the statement within its transaction, from the change table column ` + "`__$command_id`" + `. Use ` + "`command_id`" + ` for ordering changes that share the same ` + "`lsn`" + `. Not present on snapshot (` + "`read`" + `) messages.)
