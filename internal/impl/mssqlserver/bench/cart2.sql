...
PRINT CONCAT('Inserting test data into dbo.cart2 (', @cart_total, ' rows)...');
...
BEGIN
    ...
    INSERT INTO dbo.cart2 WITH (TABLOCK) (name, email, info, date_of_birth, created_at, is_active, login_count, balance)
    SELECT
        CONCAT('cart-', n),                                    -- name
...
SELECT @cart_count = COUNT(*) FROM dbo.cart2;
