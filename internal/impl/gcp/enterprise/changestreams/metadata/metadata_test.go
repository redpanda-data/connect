...
func TestIntegrationStore(t *testing.T) {
	...
	e := changestreamstest.NewEmulatorHelper(t)
	g, p := testStores(e)
	tests := []struct {
		name string
		s    *Store
	...
	t.Run("GetAllPartitionsCreatedAfter", func(t *testing.T) {
		cutoff := ts.Add(-1 * time.Second)

		for _, tc := range tests {
			t.Run(tc.name, func(t *testing.T) {
				got, err := tc.s.GetAllPartitionsCreatedAfter(t.Context(), cutoff)
				if err != nil {
					t.Fatal(err)
				}
				...
				if diff := cmp.Diff(got, want); diff != "" {
					t.Errorf("GetAllPartitionsCreatedAfter() mismatch (-want +got):\n%s", diff)
				}
			})
		}
	})
