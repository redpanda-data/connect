...
func TestIntegrationSubscriberStartReturnsErrorOnPartitionError(t *testing.T) {
	...
	mq.ExpectQuery("partition1").Run(func(args mock.Arguments) {
		...
		case <-ctx.Done():
		case <-time.After(time.Second):
			// t.Errorf (not Fatalf) — this callback runs in a subscriber goroutine,
			// and FailNow is undefined from non-test goroutines.
			t.Errorf("timed out waiting for partition1 to be aborted")
		}
	}).Return(context.Canceled)
	...
	mq.ExpectQuery("partition2").Run(func(_ mock.Arguments) {
		...
		case <-time.After(time.Second):
			t.Errorf("timed out waiting for partition1 to start")
		}
	}).Return(testErr)
