...
func (s *Subscriber) handleRootPartitions(ctx context.Context, pm metadata.PartitionMetadata, cr ChangeRecord) error {
...
func (s *Subscriber) detectNewPartitions(ctx context.Context) error {
	...
	pms, err := s.store.GetAllPartitionsCreatedAfter(ctx, minWatermark)
	if err != nil {
		return err
	}
	if len(pms) == 0 {
...
func (s *Subscriber) waitForParentPartitionsToFinish(ctx context.Context, pm metadata.PartitionMetadata) {
	for {
		...
		if err != nil {
			s.log.Errorf("Error while checking parent partitions: %v", err)
		}
		if ok {
			return
		}
...
func (s *Subscriber) queryChangeStream(ctx context.Context, partitionToken string) error {
	...
	if err := s.querier.query(ctx, pm, s.handleChangeRecord); err != nil {
...
func (s *Subscriber) handleChangeRecord(ctx context.Context, pm metadata.PartitionMetadata, cr ChangeRecord) error {
	tr := timeRange{
		cur: pm.StartTimestamp,
		end: pm.EndTimestamp,
	}

	if err := s.handleDataChangeRecords(ctx, pm, cr, &tr); err != nil {
		return err
	}
	...
	if err := s.handleChildPartitionsRecords(ctx, pm, cr, &tr); err != nil {
		return err
	}

	if tr.now().Compare(lastUpdatedWatermark) != 0 {
		if err := s.store.UpdateWatermark(ctx, pm.PartitionToken, tr.now()); err != nil {
...
func (s *Subscriber) handleDataChangeRecords(
	ctx context.Context,
	pm metadata.PartitionMetadata,
	cr ChangeRecord,
	tr *timeRange,
	...
	for _, dcr := range cr.DataChangeRecords {
		if !tr.tryClaim(dcr.CommitTimestamp) {
			s.log.Errorf("%s: failed to claim data change record timestamp: %v, current: %v",
				pm.PartitionToken, dcr.CommitTimestamp, tr.now())
			continue
		}

		s.log.Debugf("%s: data change record: table: %s, modification type: %s, commit timestamp: %v",
			pm.PartitionToken, dcr.TableName, dcr.ModType, dcr.CommitTimestamp)
		if err := s.cb(ctx, dcr); err != nil {
...
func (s *Subscriber) handleChildPartitionsRecords(
	ctx context.Context,
	pm metadata.PartitionMetadata,
	cr ChangeRecord,
	tr *timeRange,
	...
	for _, cpr := range cr.ChildPartitionsRecords {
		if !tr.tryClaim(cpr.StartTimestamp) {
			s.log.Errorf("%s: failed to claim child partition record timestamp: %v, current: %v",
				pm.PartitionToken, cpr.StartTimestamp, tr.now())
			continue
		}
		...
		for _, cp := range cpr.ChildPartitions {
			if cp.isSplit() {
				s.log.Infof("Detected partition split for partition %s", cp.ParentPartitionTokens[0])
			}
			s.log.Debugf("%s: child partition: token: %s, parent partition tokens: %+v",
				pm.PartitionToken, cp.Token, cp.ParentPartitionTokens)
			childPartitions = append(childPartitions,
				cp.toPartitionMetadata(cpr.StartTimestamp, pm.EndTimestamp, pm.HeartbeatMillis))
		}
		if err := s.store.Create(ctx, childPartitions); err != nil {
