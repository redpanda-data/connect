// Code generated by rpg; DO NOT EDIT.
...
import (
	...
	v1 "github.com/redpanda-data/connect/v4/internal/impl/notion/api/v1"
...
func baseProcessorFromParsed(conf *service.ParsedConfig, mgr *service.Resources) (p baseProcessor, err error) {
	...
	resLabel, err := conf.FieldString(fieldClient)
	if err != nil {
		return p, err
	}

	res, ok := mgr.AccessResource("notion", resLabel)
	if !ok {
		return p, fmt.Errorf("notion resource %q not found", resLabel)
	}

	client, ok := res.(notionClient)
	if !ok {
		return p, fmt.Errorf("notion resource %q: expected notionClient, got %T", resLabel, res)
	}
	p.notionClient = client
	...
	if p.responseKey, err = conf.FieldString(fieldResponseKey); err != nil {
		return p, err
	}
...
// Auth and service-specific headers are handled by the resource's HTTP transport.
...
func (p *baseProcessor) handleResponse(msg *service.Message, originalStructured any, res *apiResponse) error {
	envelope := map[string]any{
		p.responseKey: map[string]any{
...
// Req must be a struct type.
func marshalJSONRequest[Req any, O interface {
	*Req
	jsonBody
}](msg *service.Message, mapping *bloblang.Executor) (io.Reader, error) {
	if mapping != nil {
...
func completeFileUploadConfig() *service.ConfigSpec {
	return service.NewConfigSpec().
		...
		Fields(
			service.NewStringField(fieldClient).
				Description("The notion resource to use for API requests."),
			...
			service.NewStringField(fieldResponseKey).
				Default("response").
				Description("The key used to nest the API response in the output message envelope."),
...
func (p *completeFileUploadProcessor) processMessage(ctx context.Context, idx int, batch service.MessageBatch) error {
	...
	return p.handleResponse(msg, originalStructured, res)
}
