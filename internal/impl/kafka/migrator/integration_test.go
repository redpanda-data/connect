...
func startMigrator(t *testing.T, src, dst EmbeddedRedpanda, cb service.MessageHandlerFunc) {
	t.Helper()
...
`
	...
	data := struct {
		Src   EmbeddedRedpanda
		Dst   EmbeddedRedpanda
		Topic string
	}{
		Src:   src,
		Dst:   dst,
...
func startMigratorAndWaitForMessages(t *testing.T, src, dst EmbeddedRedpanda, numMessages int) {
	done := make(chan struct{})
	...
	for range numMessages {
		...
		case <-done:
			continue
		case <-time.After(redpandaOpTimeout):
