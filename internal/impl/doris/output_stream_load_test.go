...
func TestStreamLoadURLsJoinPath(t *testing.T) {
	endpoints, err := streamLoadURLs([]string{"http://fe:8030/root//"}, "db name", "tbl")
	require.NoError(t, err)
	require.Len(t, endpoints, 1)

	assert.Equal(t, "http://fe:8030/root/api/db%20name/tbl/_stream_load", endpoints[0].String())
}
