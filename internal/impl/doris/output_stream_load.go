...
const (
	...
	// Doris Stream Load reports outcomes as Status and ExistingJobStatus strings
	// instead of numeric codes, so response classification intentionally matches
	// the documented values.
	// See: https://doris.apache.org/docs/3.x/data-operate/import/import-way/stream-load-manual
...
func streamLoadURLs(baseURLs []string, database, table string) ([]*url.URL, error) {
	...
	for _, baseURL := range baseURLs {
		...
		endpoints = append(endpoints, u.JoinPath("api", database, table, "_stream_load"))
	}
