...
func (m *scriptedMockDeps) deps(chunkSize int) Deps {
	return Deps{
		ResolvePrimaryKey: func(_ context.Context, table TableID) ([]string, error) {
			return m.tables[table.String()].pkCols, nil
		},
		ResolveMaxKey: func(_ context.Context, table TableID, _ []string, _ string) (PrimaryKey, error) {
			return m.tables[table.String()].maxPK, nil
		},
		ResolveWatermark: func(context.Context) (Watermark, error) {
		...
		FetchChunk: func(_ context.Context, table TableID, _ []string, _ string, _ []any) ([]Row, error) {
			key := table.String()
			mt := m.tables[key]
			start := m.cursor[key]
...
func TestCoordinator_ResumeRefetchesUnflushedChunk(t *testing.T) {
	...
	deps := Deps{
		ResolvePrimaryKey:     func(context.Context, TableID) ([]string, error) { return []string{"id"}, nil },
		ResolveMaxKey:         func(context.Context, TableID, []string, string) (PrimaryKey, error) { return maxPK, nil },
		...
		FetchChunk: func(_ context.Context, _ TableID, _ []string, _ string, args []any) ([]Row, error) {
			var lower PrimaryKey
			if len(args) > 1 {
				lower = PrimaryKey{args[0]}
			}
	...
	cfg := Config{Tables: []TableID{tableA}, ChunkSize: chunkSize, Deps: deps}
...
func TestCoordinator_ConfigValidation(t *testing.T) {
	...
	t.Run("zero chunk size", func(t *testing.T) {
		_, err := NewCoordinator(Config{ChunkSize: 0, Deps: validDeps}, nil)
		require.Error(t, err)
	})

	t.Run("negative chunk size", func(t *testing.T) {
		_, err := NewCoordinator(Config{ChunkSize: -1, Deps: validDeps}, nil)
		require.Error(t, err)
	})
	...
	t.Run("missing dep", func(t *testing.T) {
		deps := validDeps
		deps.FetchChunk = nil
		_, err := NewCoordinator(Config{ChunkSize: 1, Deps: deps}, nil)
		require.Error(t, err)
