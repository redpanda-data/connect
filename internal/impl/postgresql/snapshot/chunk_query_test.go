...
func TestBuildChunkQueryFirstChunkSingleColumnPK(t *testing.T) {
	...
	query, args, err := buildChunkQuery(table, []string{"id"}, nil, PrimaryKey{100}, 500)
...
func TestBuildChunkQuerySubsequentChunkCompositePK(t *testing.T) {
	table := TableID{Schema: "public", Table: "line_items"}
...
func TestBuildChunkQueryNoPKColumnsIsError(t *testing.T) {
	...
	_, _, err := buildChunkQuery(table, nil, nil, PrimaryKey{1}, 500)
	require.Error(t, err)
}
