...
func TestNeonPostgresCDCReplication(t *testing.T) {
	t.Skip()
	tmpDir := t.TempDir()

	dbhost := ""     // neon db host
	dbport := ""     // neondb port
	dbPassword := "" // neondb password
	dbUser := ""     // neondb user
	dbName := ""     // neondb name

	databaseURL := fmt.Sprintf("user=%s password=%s dbname=%s sslmode=require host=%s port=%s", dbUser, dbPassword, dbName, dbhost, dbport)
	...
	assert.Equal(t, "logical", walLevel)

	_, err = db.Exec("DROP TABLE IF EXISTS flights;")
	require.NoError(t, err)
	_, err = db.Exec("DROP TABLE IF EXISTS flights_non_streamed;")
	require.NoError(t, err)
	...
	dbConnOptions := "" // neondb connection options for the endpoint
...
pg_stream:
    ...
    tls: require
    stream_snapshot: true
    pg_conn_options: endpoint=%s
    decoding_plugin: wal2json
    database: %s
    tables:
       - flights
`, dbhost, dbUser, dbPassword, dbport, dbConnOptions, dbName)
