	...
	if len(connExecResult) > 0 && len(connExecResult[0].Rows) > 0 {
		...
		if config.ForceSnapshot && config.StreamOldData {
			...
			go func() {
				...
				// avoiding a race between the snapshot's own rows and newly
				// streamed WAL changes. Connect already cleared the pending
				// signal before this attempt started, so a crash here does not
				// retry the re-snapshot.
				select {
