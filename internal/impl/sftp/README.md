...
Now you should be able to interact with the SFTP server via `localhost:2022` with user `admin` and password `password`.

Note: You'll have to also fetch the server's

You should also be able to access the SFTPGo web UI via http://localhost:8080 with user `admin` and password `password`.
