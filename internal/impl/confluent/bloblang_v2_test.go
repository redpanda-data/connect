...
func TestWithSchemaRegistryHeaderV2(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			mapping:          `output = with_schema_registry_header(123, "hello world")`,
			expectedSchemaID: 123,
			expectedText:     "hello world",
		},
		{
			name:             "zero schema id",
			mapping:          `output = with_schema_registry_header(0, "test")`,
			expectedSchemaID: 0,
			expectedText:     "test",
		},
		{
			name:             "max uint32 schema id",
			mapping:          `output = with_schema_registry_header(4294967295, "test")`,
			expectedSchemaID: 4294967295,
			expectedText:     "test",
		},
		{
			name:             "empty message",
			mapping:          `output = with_schema_registry_header(456, "")`,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			e, err := bloblangv2.Parse(test.mapping)
...
func TestWithSchemaRegistryHeaderV2Errors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			mapping:       `output = with_schema_registry_header(-1, "test")`,
			expectedError: "schema ID must be between 0 and 4294967295",
		},
		{
			name:          "schema id too large",
			mapping:       `output = with_schema_registry_header(4294967296, "test")`,
			expectedError: "schema ID must be between 0 and 4294967295",
		},
	}
