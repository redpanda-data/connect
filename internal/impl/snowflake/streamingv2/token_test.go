...
// fixedRSAKeyPEM is a throwaway 2048-bit RSA test vector (PKCS#8), generated
// with `openssl genrsa 2048 | openssl pkcs8 -topk8 -nocrypt`, so
// PublicKeyFingerprint can be checked against wantFixedKeyFingerprint, which
// was computed independently with openssl. A runtime-generated key could not
// have a hardcoded expected value.
//
// NOT A CREDENTIAL: it authenticates to nothing and is used nowhere else.
// Real account keys are read at run time from SNOWFLAKE_PRIVATE_KEY_FILE.
//
// gitleaks:allow
// trufflehog:ignore
//
...
func TestScopedTokenExchangeAndCaching(t *testing.T) {
	...
	second, err := ts.scoped(t.Context())
	if err != nil {
...
func TestScopedTokenRefreshesBeforeExpiry(t *testing.T) {
	...
	if _, err := ts.scoped(t.Context()); err != nil {
...
// TestScopedTokenThunderingHerd: N concurrent callers on an absent token
// produce exactly one exchange. The server sleeps per request so a second
// concurrent exchange cannot hide behind scheduling.
func TestScopedTokenThunderingHerd(t *testing.T) {
	...
	for i := range n {
		...
		go func(i int) {
			defer wg.Done()
			<-start
			tokens[i], errs[i] = ts.scoped(t.Context())
		}(i)
	}
	close(start)
	wg.Wait()
...
// TestTokenSourceScopeCarriesHostAndOptionalRole: the exchange scope is the
// ingest host plus "session:role:<role>" when a role is configured. Snowflake
// enforces the role at the pipe, so it has to be in the scope.
