...
// TestControlPlaneJWTIsCachedAcrossCalls: every control-plane request in a
// session carries the same JWT rather than minting (RSA-signing) a new one.
func TestControlPlaneJWTIsCachedAcrossCalls(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/oauth/token":
			...
			// not asserted here, this test is about the
	...
	if _, err := c.ChannelStatuses(t.Context(), []string{"some-channel"}); err != nil {
...
// TestConnectRejectsTokenTTLAtOrBelowRefreshMargin: a TokenTTL that leaves no
// room for the refresh margin is rejected by newTokenSource's guard.
...
// TestConnectPreflightSucceedsOnNormalStatus: the pre-flight only checks that
// the pipe-scoped status call succeeds, not what statuses it returns.
...
// TestConnectFailsLoudlyOnInvalidPipePreflight: a non-2xx from the pre-flight
// fails Connect with an error naming the status and the qualified pipe.
...
// TestConnectPreflightNotReachedOnEarlierFailure: a discovery failure is
// reported as itself, and the pre-flight is never attempted after it.
...
// TestConnectPreflightRetriesRateLimitThenSucceeds: a 429 on the pre-flight is
// retried rather than reported as a bad pipe.
