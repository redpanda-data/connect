...
func TestIsBackpressureClassifiesStatusCodes(t *testing.T) {
	for _, tc := range []struct {
		...
		// the code rather than the status is what makes both retryable, so
		// both are pinned here -- the 500 case is the one that was previously
		// fatal on every path, including the status poll.
...
// TestChannelOpen409sAreMutuallyExclusive asserts directly, for both
// channel-open 409 error_codes, that exactly one of IsOpenChannelInProgress
// and IsUncommittedDataConflict fires -- never both, never neither. This is
// the explicit form of the distinction OpenChannel's retry decision depends
// on (channel.go: only the first is retried): a classifier that lumped the
// two together, e.g. by falling ERR_CHANNEL_HAS_UNCOMMITTED_DATA through to
// the same branch as ERR_OPEN_CHANNEL_IN_PROGRESS, would pass a test that
// only checked one predicate per case but fails this one, since it asserts
// both predicates on both codes in the same test.
...
// TestHTTP404NeverClassifiesAsRetryable establishes that a 404 cannot reach a
// retry decision in this client today, and pins that fact so it doesn't
// silently regress.
//
// The reference SDK has its own gate for promoting a misclassified
// 404 to retryable: (empty error_code AND empty message) OR html
// content-type, with api_name acting as a one-item blocklist
// (get_subdomain_name) rather than an allowlist of insert_rows. That gate is
// NOT reimplemented here, and this test is why: Client.do (client.go) turns
// every non-2xx, including 404, into the same *httpStatusError as any other
// status, with no 404-specific branch anywhere in this package. The only
// retry loop in this client, OpenChannel (channel.go), decides to retry on
// exactly two conditions -- statusCodeOf(err) == 429, or
// IsOpenChannelInProgress(err) (which itself requires status == 409) -- and a
// 404 satisfies neither. IsBackpressure is exported but has no production
// caller in this package (only IsOpenChannelInProgress feeds a retry
// decision today), so even its body-driven backpressureErrorCodes signal
// cannot promote a 404 into a retry.
//
// Concretely: neither an error-enveloped 404 body (mirroring the SDK's own
// test_404_with_error_envelope_does_not_retry) nor a bare 404 classifies as
// backpressure, an open-in-progress conflict, or an uncommitted-data
// conflict -- there is no path from "404" to "retry" for any classifier this
// package exposes.
//
// The SDK's 404 gate becomes relevant here only if this client later adds
// its own 404 classification (e.g. to retry a 404 from hostname discovery
// during Connect, the same case get_subdomain_name covers in the SDK) --
// until then, importing it would be a classifier with no caller.
