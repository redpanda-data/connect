...
// conformanceClient builds a Client pointed entirely at srv via BaseURL
// (both planes); TestEndpointsUseTheCorrectPlane avoids BaseURL for that
// reason.
...
func TestInsertRowsURLMatchesSDKShape(t *testing.T) {
	...
	ch, err := c.OpenChannel(t.Context(), "DB.SC.P-p0")
	if err != nil {
		t.Fatalf("OpenChannel: %v", err)
	}
	if _, err := ch.AppendRows(t.Context(), [][]byte{[]byte(`{"a":1}`)}, "10", "12"); err != nil {
...
func TestInsertRowsURLRendersEmptyStartTokenAsPresentButEmpty(t *testing.T) {
	...
	if _, err := ch.AppendRows(t.Context(), [][]byte{[]byte(`{"a":1}`)}, "", "12"); err != nil {
		t.Fatalf("AppendRows: %v", err)
	}
	raw := rowsQuery(t, seen)
...
// TestOffsetTokensAreEscapedDeliberatelyUnlikeTheSDK: offset tokens are
// percent-encoded in the query string (the reference SDK does not escape). An
// operator-supplied token containing & or = would otherwise split the query
// and change which offset the server records.
func TestOffsetTokensAreEscapedDeliberatelyUnlikeTheSDK(t *testing.T) {
	...
	if _, err := ch.AppendRows(t.Context(), [][]byte{[]byte(`{"a":1}`)}, "a b&c=d", "a b&c=d"); err != nil {
		t.Fatalf("AppendRows: %v", err)
	}
	raw := rowsQuery(t, seen)
...
// TestAppendRowsWithAllEmptyBatchMakesNoRequestAndLeavesTokenUntouched: an
// all-empty batch makes no request, reports sent=false, and leaves the
// continuation token as it was.
func TestAppendRowsWithAllEmptyBatchMakesNoRequestAndLeavesTokenUntouched(t *testing.T) {
	...
	sent, err := ch.AppendRows(t.Context(), [][]byte{{}, []byte("   "), []byte("\n\t")}, "1", "1")
	if err != nil {
	...
	sent, err = ch.AppendRows(t.Context(), [][]byte{[]byte(`{"a":1}`)}, "2", "2")
	if err != nil {
...
// TestEndpointsUseTheCorrectPlane: discovery and the token exchange go to the
// account host; channel and row endpoints go to the discovered ingest host.
// Two servers are used because BaseURL would override both planes.
...
// TestRequestHeadersMatchProtocol pins the header protocol: an explicit
// Accept on every request (the API rejects its absence), KEYPAIR_JWT on the
// account host vs OAUTH on the ingest host, no token-type header on the
// /oauth/token exchange, and application/x-ndjson on rows.
