...
// TestIntegrationSnowflakeStreamingPipe exercises the snowflake_streaming_pipe
// output against a real Snowflake account. It is the migrated form of the
// SSv2 demo repo's own integration_test.go, adapted to this repo's simpler
// existing integration-test pattern (see streaming_v2_ithelper_test.go's doc
// comment for what changed and why).
//
// Covers:
//   - Tier1_ValuesLandAndDedupHolds: rows produced land in the destination
//     table, decimal precision survives (12.50, not 13), and re-sending the
//     exact same offset tokens against a fresh stream instance inserts
//     nothing new (the output's own offset-token dedup, not any Kafka
//     consumer-group mechanism).
//   - Tier2_EnrichmentAndNullPreservation / Tier2b_WhereClauseFiltering:
//     the pipe-level stream-static-join enrichment and WHERE-clause
//     filtering Streaming Transformations, gated behind
//     ENABLE_STREAMING_STATIC_JOIN / ENABLE_SNOWPIPE_STREAMING_WHERE_CLAUSE.
//     A 003001 (42501) or 002098 (0A000) error applying the tier 2 fixture is
//     treated as "feature unavailable on this account, skip tier 2" rather
//     than a hard failure -- see isTier2Unavailable.
func TestIntegrationSnowflakeStreamingPipe(t *testing.T) {
	...
	// Fixtures are applied lazily, at most once each, from inside whichever
	// subtest needs them first, rather than eagerly at the top of the test:
	// a fixture failure must fail only the subtests that depend on it, not
	// every subtest in the suite. Mirrors runIntegrationSuite's own
	// tier1Once/tier2Once in the source repo.
	...
	t.Run("Tier1_ValuesLandAndDedupHolds", func(t *testing.T) {
		...
		// Re-sending the exact same offset tokens against a *fresh* stream
		// instance (a fresh channel pool, fresh v2.Client) must insert
		// nothing new: the channel name is deterministic per pipe+partition,
		// so the new stream reopens the same Snowflake channel and
		// re-presents the same offset tokens, which is what actually
		// exercises the output's own offset-token dedup against Snowflake's
		// committed state -- not any in-process cache that a second stream
	...
	t.Run("Tier2_EnrichmentAndNullPreservation", func(t *testing.T) {
		...
		// b-2 is deliberately absent from the dimension table (see
