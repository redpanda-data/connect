...
func init() {
	diffSpec := bloblangv2.NewPluginSpec().
		...
		Version("4.25.0").
		Param(bloblangv2.NewAnyParam("other").Description("The value to compare against the current value. Can be any structured data (object or array)."))

	bloblangv2.MustRegisterMethod("diff", diffSpec, func(args *bloblangv2.ParsedParams) (bloblangv2.Method, error) {
	...
	patchSpec := bloblangv2.NewPluginSpec().
		Category("Object & Array Manipulation").
		...
		Version("4.25.0").
		Param(bloblangv2.NewAnyParam("changelog").Description("The changelog array to apply. Should be in the format returned by the diff method, containing Type, Path, From, and To fields for each change."))

	bloblangv2.MustRegisterMethod("patch", patchSpec, func(args *bloblangv2.ParsedParams) (bloblangv2.Method, error) {
