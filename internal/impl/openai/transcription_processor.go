...
To learn more about audio transcription, see the: https://platform.openai.com/docs/guides/speech-to-text[OpenAI API documentation^].`).
		...
		Fields(
			audioFileField(otspFieldFile, "transcribe"),
			service.NewInterpolatedStringField(otspFieldLang).
				...
				Optional().
				Advanced(),
			audioPromptField(otspFieldPrompt),
		)
}

// audioFileField returns the field that selects the audio to send to the
// OpenAI API, shared by the transcription and translation processors. The verb
// names the operation, for example "transcribe" or "translate".
func audioFileField(name, verb string) *service.ConfigField {
	return service.NewBloblangField(name).
		Description("The audio file object (not file name) to " + verb + ", in one of the following formats: `flac`, `mp3`, `mp4`, `mpeg`, `mpga`, `m4a`, `ogg`, `wav`, or `webm`.").
		ShortDescription("The audio file object to " + verb + ". Supports flac, mp3, mp4, mpeg, mpga, m4a, ogg, wav and webm.")
}

// audioPromptField returns the optional prompt field shared by the
// transcription and translation processors.
func audioPromptField(name string) *service.ConfigField {
