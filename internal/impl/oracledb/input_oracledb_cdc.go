...
- rs_id: The LogMiner record set ID from ` + "`V$LOGMNR_CONTENTS.RS_ID`" + `, identifying the redo record that produced the change. Together with ` + "`ssn`" + ` it identifies one row change, except for bulk deletes, where all rows of one redo record share the same values. Not present on snapshot (` + "`read`" + `) messages, nor on LOB-only synthetic updates.
