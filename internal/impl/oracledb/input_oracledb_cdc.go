...
The checkpoint also advances while the monitored tables are idle. After each mining cycle, when every delivered batch is acknowledged, the input saves the end of the mined range. The saved SCN stays before the start of any transaction that is still open, and the input does not save it while LOB data of an open transaction is in progress. If the batcher holds rows that are not flushed yet, the input waits until they flush. With ` + "`" + ociFieldBatching + ".count`" + ` and no ` + "`" + ociFieldBatching + ".period`" + `, rows can stay in the batcher until more rows arrive, and the checkpoint does not advance during that time. Set a ` + "`period`" + ` to prevent this. Redo log retention must be longer than the longest plausible pipeline outage and the longest open transaction. If the checkpointed SCN is no longer available when activity resumes or the pipeline restarts, the input cannot resume and repeatedly fails with ORA-01291. Alert on a stagnant checkpoint SCN or repeated ORA errors.
