...
- row_seq: The position of the row change among the rows that share the same ` + "`rs_id`" + ` and ` + "`ssn`" + ` in the transaction, starting at 0 in redo order. Oracle reports the same ` + "`rs_id`" + ` and ` + "`ssn`" + ` for every row of an array DML redo record, such as a bulk ` + "`DELETE`" + ` of up to 255 rows, so this field tells those rows apart. It is 0 for every other change. Compare it numerically. Not present whenever ` + "`rs_id`" + ` is not present.
