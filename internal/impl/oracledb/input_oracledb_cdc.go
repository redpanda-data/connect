...
- user_name: The Oracle database user that executed the change, sourced from ` + "`V$LOGMNR_CONTENTS.USERNAME`" + `. Not present for internal/recursive transactions or when the underlying ` + "`USERNAME`" + ` value is unavailable in the mined redo (depending on redo/dictionary configuration), and not present on snapshot (` + "`read`" + `) messages.
