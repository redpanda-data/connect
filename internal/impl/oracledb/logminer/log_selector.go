...
func (s *logFileSelector) selectForSession(files []*LogFile, openThreads []int, dbCurrentSCN, maxRedoLogSizeInBytes uint64) (selected []*LogFile, endSCN uint64, capped bool, err error) {
	...
	if truncated && slices.Equal(budgetKeys, s.prevKeys) {
		...
		// MinRedoVolumeGrowthCeiling guards against a permanent stall: a
		// budget of 1 always reselects its own single file forever (its
		// NextSCN-1 boundary re-qualifies it next cycle), so it can only
		// ever escape via growth - clamping the ceiling to 1 as well would
		// trap it.
