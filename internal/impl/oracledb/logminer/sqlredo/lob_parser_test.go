...
func TestParseSelectLobLocator(t *testing.T) {
	tests := []struct {
		...
		{
			// New Oracle format: all three LOB types declared; SELECT INTO names the variable used.
			name:       "new Oracle format - CLOB column (loc_c)",
		...
		{
			// New Oracle format: NCLOB column selected into loc_nc.
			name:       "new Oracle format - NCLOB column (loc_nc)",
		...
		{
			// Old Oracle format: single variable lob_1.
			name:       "old Oracle format (lob_1)",
...
func TestParseLobWrite(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "CLOB write",
			sql:        "dbms_lob.write(:1, 5, 1, 'Hello');",
			isBinary:   false,
			wantData:   []byte("Hello"),
			wantOffset: 1,
			wantLength: 5,
		...
		{
			name:       "BLOB write",
			sql:        "dbms_lob.write(:1, 5, 1, HEXTORAW('48656C6C6F'));",
			isBinary:   true,
			wantData:   []byte("Hello"),
			wantOffset: 1,
			wantLength: 5,
		...
		{
			// New Oracle format: data in buffer variable, plain PL/SQL var as LOB locator.
			name:       "new format - CLOB buffer assignment",
		...
		{
			name:       "new format - BLOB HEXTORAW buffer assignment",
		...
		{
			name:       "new format - escaped quote in CLOB",
		...
		{
			name:     "old format - BLOB missing HEXTORAW",
			sql:      "dbms_lob.write(:1, 5, 1, 'hello');",
			isBinary: true,
			wantErr:  true,
		},
