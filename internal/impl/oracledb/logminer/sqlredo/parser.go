...
// ParsedStatement holds a pre-validated SQL_REDO string ready for value extraction.
type ParsedStatement struct{ sql string }

// ParseSQLCommand validates the SQL and returns a ParsedStatement for use with ExtractValuesFromAST.
func ParseSQLCommand(sql string) (*ParsedStatement, error) {
	if _, _, err := ScanSQLCommand(sql, nil); err != nil {
		return nil, fmt.Errorf("parsing sql command from logminer: %w", err)
	}
	return &ParsedStatement{sql: sql}, nil
}

// ExtractValuesFromAST extracts column→value mappings from a parsed statement.
// Returns newValues (INSERT/UPDATE) and oldValues (UPDATE/DELETE WHERE clauses).
// When converter is non-nil, bare values are passed through ConvertValue.
func ExtractValuesFromAST(stmt *ParsedStatement, converter *OracleValueConverter) (newValues, oldValues map[string]any, err error) {
	return ScanSQLCommand(stmt.sql, converter)
}
