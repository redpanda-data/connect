...
func TestIsResultsClosed(t *testing.T) {
	tests := []struct {
		name    string
		content string
		expect  bool
		...
		{"closed", "some content\n</results>\n", true},
		{"closed no trailing newline", "some content\n</results>", true},
		{"closed with trailing whitespace", "some content\n</results>  \n\n", true},
		{"not closed", "some content\n=== RUN TestFoo", false},
		{"empty", "", false},
		{"partial tag", "some content\n</result>", false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.expect, isResultsClosed(tt.content))
		})
	}
}
