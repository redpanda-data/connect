...
func init() {
	Constructors[TypeQueueStorage] = TypeSpec{
...
`,
		sanitiseConfigFunc: func(conf Config) (interface{}, error) {
			return sanitiseWithBatch(conf.QueueStorage, conf.QueueStorage.Batching)
		},
		Async:   true,
		Batches: true,
		FieldSpecs: docs.FieldSpecs{
			...
			docs.FieldAdvanced(
				"ttl", "The TTL of each individual msg as a duration string. Defaults to 0, meaning no TTL is set",
				"60s", "5m", "36h",
			),
...
func NewAzureQueueStorage(conf Config, mgr types.Manager, log log.Modular, stats metrics.Type) (Type, error) {
	s, err := writer.NewAzureQueueStorage(conf.QueueStorage, log, stats)
	if err != nil {
		return nil, err
	}
	var w Type
	if conf.QueueStorage.MaxInFlight == 1 {
		w, err = NewWriter(
			TypeQueueStorage, s, log, stats,
		)
	} else {
		w, err = NewAsyncWriter(
			TypeQueueStorage, conf.QueueStorage.MaxInFlight, s, log, stats,
		)
	}
	if err != nil {
		return w, err
	}
	return newBatcherFromConf(conf.QueueStorage.Batching, w, mgr, log, stats)
}
