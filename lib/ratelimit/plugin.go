...
// PluginConstructor is a func that constructs a Benthos ratelimit plugin. These
...
// pluginSpecs is a map of all rate limit plugin type specs.
...
// constructed similar to regular rate limits. If configuration is not needed
// for this plugin then configConstructor can be nil. A constructor for the
// plugin itself must be provided.
func RegisterPlugin(
	typeString string,
	...
	spec.constructor = func(conf Config, mgr types.Manager, log log.Modular, stats metrics.Type) (types.RateLimit, error) {
		return constructor(conf.Plugin, mgr, log, stats)
	}
	spec.confConstructor = configConstructor
	pluginSpecs[typeString] = spec
	plugins.Add(typeString, "rate_limit")
}
...
func PluginDescriptions() string {
	...
	buf := bytes.Buffer{}
	buf.WriteString("Rate Limit Plugins\n")
