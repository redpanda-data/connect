...
func FieldSpec() docs.FieldSpec {
	return docs.FieldAdvanced(
		...
		).AtVersion("3.45.0").HasType(docs.FieldBool).HasDefault(false),
		...
		docs.FieldCommon(
			...
			docs.FieldCommon(
				"cert", "A plain text certificate to use.",
			).HasType(docs.FieldString).HasDefault(""),

			docs.FieldCommon(
				"key", "A plain text certificate key to use.",
			).HasType(docs.FieldString).HasDefault(""),

			docs.FieldCommon(
				"cert_file", "The path to a certificate to use.",
			).HasType(docs.FieldString).HasDefault(""),

			docs.FieldCommon(
				"key_file", "The path of a certificate key to use.",
			).HasType(docs.FieldString).HasDefault(""),
		),
	)
}
