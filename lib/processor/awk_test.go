...
func TestAWK(t *testing.T) {
	...
	tests := []jTest{
		...
		},
		{
			...
			codec:   "none",
			program: `{ json_array_append("obj.foo", "hello world") }`,
			input:   `{}`,
			output:  `{"obj":{"foo":["hello world"]}}`,
		},
		{
		...
		},
		{
			...
			codec:   "none",
			program: `{ json_array_append_int("obj.foo", 1) }`,
			input:   `{}`,
			output:  `{"obj":{"foo":[1]}}`,
		},
		{
			...
			codec:   "none",
			program: `{ json_array_append_float("obj.foo", 1.2) }`,
			input:   `{}`,
			output:  `{"obj":{"foo":[1.2]}}`,
		},
		{
			...
			codec:   "none",
			program: `{ json_array_append_bool("obj.foo", 1) }`,
			input:   `{}`,
			output:  `{"obj":{"foo":[true]}}`,
		},
		{
			...
			codec:   "none",
			program: `{ json_array_append_bool("obj.foo", 0) }`,
			input:   `{}`,
