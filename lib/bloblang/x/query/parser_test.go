...
func TestFunctionParserErrors(t *testing.T) {
	tests := map[string]struct {
		...
		"bad args 2": {
			input:      `json("foo`,
			deprecated: true,
			err:        `char 5: required one of: [boolean number quoted-string match function null array object field-path]`,
		},
		"bad args 3": {
			input: `json(`,
			err:   `char 5: required one of: [boolean number quoted-string match function null array object field-path]`,
		...
		"bad args 4": {
			input: `json(0,`,
			err:   `char 7: required one of: [boolean number quoted-string match function null array object field-path]`,
		},
		"bad args 5": {
			input:      `json`,
			deprecated: true,
		...
		"bad operators": {
			input: `json("foo") + `,
			err:   `char 14: expected one of: [match function boolean number quoted-string null array object field-path]`,
		},
		"bad expression": {
			input: `(json("foo") `,
		...
		"bad expression 2": {
			input: `(json("foo") + `,
			err:   `char 15: expected one of: [match function boolean number quoted-string null array object field-path]`,
		},
		"bad expression 3": {
		...
		"bad method args 2": {
			input: `json("foo").from(`,
			err:   `char 17: required one of: [boolean number quoted-string match function null array object field-path]`,
		},
		"bad method args 3": {
			input: `json("foo").from()`,
		...
		"gibberish": {
			input: `json("foo").(=)`,
			err:   `char 13: required one of: [match function boolean number quoted-string null array object field-path]`,
		},
		"gibberish 2": {
			input: `json("foo").(1 + )`,
			err:   `char 17: required one of: [match function boolean number quoted-string null array object field-path]`,
		},
		"bad match": {
