...
These changes are intended to reflect the actual behaviour of the methods, where they replace all occurrences of the arguments. In each of these cases the old name returns a parse error in order to prevent unexpected behaviour, but in the future the old names will likely be reintroduced with appropriate behaviour, where they only replace the first occurrence by default.
...
Finally, `prometheus` is now the default metrics type and also exposes timing metrics as histograms by default, you can switch this back to a summary by setting `use_histogram_timing` to `false`.
