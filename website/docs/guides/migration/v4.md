...
## Parquet Processors Scheduled Removal

The `parquet` processor has been deprecated, scheduled for removal, and renamed `parquet_old`. This is due to ongoing concerns with the underlying library used for implementing this processor. A new library is being investigated and, provided it meets our standards, we will be recreating a more intuitive `parquet` processor before eventually removing this one.
