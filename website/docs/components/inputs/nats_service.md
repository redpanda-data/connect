...
:::
Register a Microservice with NATS and replies to requests on grouped endpoints.
...
input:
  ...
  nats_service:
    ...
    name: min_max # No default (required)
    version: 0.0.1
    description: Benthos Microservice
    groups: [] # No default (required)
```
...
NATS Microservice name, no whitespaces allowed
...
NATS Microservice semver based version.
...
Default queueGroup of all endpoints in this service
...
NATS Microservice group name. It should be a valid NATS subject or an empty string, but cannot contain > wildcard (as group name serves as subject prefix).
...
Default queueGroup for endpoints that overrides service queueGroup.
...
list of NATS Microservice endpoints


Type: `array`  

### `groups[].endpoints[].name`

Endpoint name, no whitespaces allowed


Type: `string`  

### `groups[].endpoints[].subject`

An optional NATS subject on which the endpoint will be registered. A subject is created by concatenating the subject provided by the user with group prefix (if applicable). If subject is not provided, name is used instead.


Type: `string`  

### `groups[].endpoints[].queue_group`

Overrides service and group queueGroup.


Type: `string`  

### `groups[].endpoints[].metadata`
