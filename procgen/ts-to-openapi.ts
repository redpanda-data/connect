...
function loadTsSource(): SourceFile {
  ...
  return project.addSourceFileAtPath(
    join(DIR, "notion-api-endpoints.ts")
  );
}
...
function extractEndpoints(sf: SourceFile): EndpointMeta[] {
  ...
  for (const vs of sf.getVariableStatements()) {
    ...
    for (const decl of vs.getDeclarations()) {
      const init = decl.getInitializerIfKind(
...
function convertTypeNode(node: TypeNode): Schema {
  ...
  switch (kind) {
    ...
    case SyntaxKind.ArrayType:
      return {
        type: "array",
        items: convertTypeNode(
          (node as any).getElementTypeNode()
        ),
      };
...
function convertTypeReference(node: TypeNode): Schema {
  ...
  if (typeName === "Record" && typeArgs.length === 2) {
    ...
    if (
      typeArgs[1].getKind() === SyntaxKind.NeverKeyword
    ) {
      return { type: "object" };
    }
  ...
  try {
    ...
    if (type.isObject()) {
      ...
      for (const prop of props) {
        ...
        if (propType) {
          if (propType.isString()) properties[prop.getName()] = { type: "string" };
          else if (propType.isNumber()) properties[prop.getName()] = { type: "number" };
          else if (propType.isBoolean()) properties[prop.getName()] = { type: "boolean" };
...
function buildRequestBodySchema(ep: EndpointMeta): Schema | null {
  ...
  if (!schemas.has(ep.paramsTypeName) && !processingTypes.has(ep.paramsTypeName)) {
...
function postProcessSchemas() {
  ...
  for (const [name, schema] of schemas) {
    ...
    const commonProps = (commonResolved?.properties as Record<string, Schema>) || {};
    ...
    for (const variant of variants) {
      ...
      const vProps = (resolved.properties as Record<string, Schema>) || {};
      for (const [k, v] of Object.entries(vProps)) {
        if (!allProperties[k]) {
          ...
          allProperties[k] = {
            ...allProperties[k],
            enum: [...new Set([...(allProperties[k].enum as any[]), ...(v.enum as any[])])],
          };
        }
      }
      variantRequiredSets.push(new Set((resolved.required as string[]) || []));
    }

    // Required: common required + intersection of variant required
    const variantRequired = variantRequiredSets.length > 0
      ? [...variantRequiredSets[0]].filter(f => variantRequiredSets.every(s => s.has(f)))
      : [];
  ...
  function flattenSchemaRecursive(schema: Schema) {
    ...
    if (schema.oneOf && Array.isArray(schema.oneOf) && (schema.oneOf as Schema[]).length >= 2) {
      ...
      if (merged) {
        const disc = schema.discriminator as
          ...
          | undefined;
        if (disc?.mapping) {
          const mergedProps = merged.properties as Record<string, Schema>;
  ...
  function detectAndMapDiscriminator(schema: Schema) {
    ...
    for (const propName of ["type", "object"]) {
      ...
      if (allHaveValue && Object.keys(mapping).length === variants.length) {
        // All variants have unique discriminator values
        // Only add mapping if we have refs
        const hasRefs = Object.values(mapping).every((v) => v.length > 0);
    ...
    if (variant.allOf) {
      ...
      for (const part of allOf) {
        if (part.$ref) {
          ...
          const refSchema = schemas.get(refName);
          if (refSchema) {
            const val = extractDiscriminatorValue(refSchema, propName);
            if (val) {
        ...
        const props = part.properties as Record<string, Schema> | undefined;
        if (props?.[propName]) {
  ...
  function mergeAllOfRefs(allOf: Schema[]): Schema | null {
    ...
    for (const part of allOf) {
      ...
      const props = refSchema.properties as
        ...
        | undefined;
      if (props) {
        Object.assign(merged.properties as Record<string, Schema>, props);
      }
...
function main() {
  ...
  console.log(`  Written to openapi.yaml (${(output.length / 1024).toFixed(0)} KB)`);
