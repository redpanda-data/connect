Use this skill when users need to create or fix Redpanda Connect pipeline configurations. Trigger when users mention "config", "pipeline", "YAML", "create a config", "fix my config", "validate my pipeline", or describe a streaming pipeline need like "read from Kafka and write to S3".
...
See `${CLAUDE_PLUGIN_ROOT}/recipes/` directory of validated patterns:
