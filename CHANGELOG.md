...
- oracledb_cdc: Fix the per-message schema metadata flipping between the snapshot and streaming paths for TIMESTAMP, INTEGER, binary and JSON columns, which caused permanent Schema Registry BACKWARD-compatibility rejections (and a failed registration round-trip per row) after a pipeline restart. Affected columns now consistently emit one type: TIMESTAMP-family → Timestamp, INTEGER/INT/SMALLINT → Decimal(38,0), NUMBER(*,s) → Decimal(38,s), negative-scale NUMBER(p,-s) → BigDecimal, BLOB/LONG RAW → ByteArray, native JSON → Any. Subjects whose registered schema carries an old losing typing need a one-time migration described in the PR — including string-typed timestamps registered from the catalog path (e.g. after a restart), string-typed BLOB/LONG RAW or JSON columns registered from the snapshot path (the incorrect side for those types), and long-typed negative-scale NUMBER columns from the catalog path. Negative-scale NUMBER columns now behave like bare NUMBER and FLOAT: BigDecimal, requiring an upstream cast/coercion for Avro encoding. Additionally, `lob_enabled: false` now correctly nulls LOB columns for connections using non-inline LOB fetching (`lob fetch=stream`/`post`), which previously did not null them. ([@Jeffail](https://github.com/Jeffail), [#4661](https://github.com/redpanda-data/connect/pull/4661))
