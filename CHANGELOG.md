...
- oracledb_cdc: Fix the per-message schema metadata flipping between the snapshot and streaming paths for TIMESTAMP, INTEGER, binary and JSON columns, which caused permanent Schema Registry BACKWARD-compatibility rejections (and a failed registration round-trip per row) after a pipeline restart. Affected columns now consistently emit one type: TIMESTAMP-family → Timestamp, INTEGER/INT/SMALLINT → Decimal(38,0), NUMBER(*,s) → Decimal(38,s), negative-scale NUMBER(p,-s) → BigDecimal, BLOB/LONG RAW → ByteArray, native JSON → Any. Subjects whose registered schema carries the old losing typing need a one-time migration described in the PR — that includes string-typed timestamps or BIG_DECIMAL NUMBER(*,s) registered from the catalog path (e.g. after a restart), AND string-typed BLOB/LONG RAW or JSON columns registered from the snapshot path, which was the incorrect side for those types. Additionally, `lob_enabled: false` now correctly nulls LOB columns for connections using non-inline LOB fetching (`lob fetch=post`/`stream`), which previously leaked CLOB content. ([@Jeffail](https://github.com/Jeffail), [#4661](https://github.com/redpanda-data/connect/pull/4661))
