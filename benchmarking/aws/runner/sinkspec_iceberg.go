...
// icebergNamespace is the Iceberg namespace (Glue database) both engines
// write to.
const icebergNamespace = "bench"

// icebergSinkSpec drains a pre-seeded Redpanda topic into an Apache Iceberg
// table (AWS Glue REST catalog + S3), head-to-head against the Kafka Connect
// Iceberg sink. Throughput is the table's committed-bytes growth, polled from
// Glue by the metric sidecar.
var icebergSinkSpec = sinkSpec{
	OutputComponent: "iceberg",
	HelperBinary:    "iceberg-tablegen",
	ArtifactPrefix:  "iceberg",
	Namespace:       icebergNamespace,
	DecorateOutput:  icebergDecorateOutput,
	ResetScript:     icebergResetScript,
	SidecarSetup:    icebergSidecarSetup,
	KCConfig:        icebergKCConfig,
}

// icebergDecorateOutput fills the catalog/storage config from TF output
// placeholders (resolved by substitutePlaceholders). The Iceberg table routes
// through the BenchNames helpers (literals built from the real SessionID) so
// the per-engine table matches exactly what ResetScript/SidecarSetup poll —
// IcebergTable dash-sanitizes for Glue while SourceTopic keeps dashes, which
// the ${BENCH_SESSION_ID} placeholder could not do consistently.
func icebergDecorateOutput(_ *Scenario, n BenchNames, cfg map[string]any) {
	cfg["catalog"] = map[string]any{
		"url":       "${GLUE_REST_URI}",
	...
	cfg["namespace"] = icebergNamespace
...
func icebergSidecarSetup(args MetricSidecarArgs, artifact string) string {
...
func icebergKCConfig(s *Scenario, outs map[string]string, n BenchNames) (KCRenderResult, error) {
	in := kcRenderInputs{
	...
	if err != nil {
		return KCRenderResult{}, fmt.Errorf("render KC iceberg config: %w", err)
	}
